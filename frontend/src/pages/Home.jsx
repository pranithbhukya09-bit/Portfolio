...
const Home = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['about', 'experience', 'projects', 'publications', 'certifications', 'skills', 'education', 'contact'];
  ...
  const experiences = [
    {
      ...
      current: true,
      description: [
        'Lead strategic planning and organizational development initiatives for the Indian Student Association',
        'Oversee operations and coordinate events to enhance member engagement and community outreach',
        'Guide team collaboration to achieve the organization\'s goals and mission effectively'
      ],
      skills: ['Team Management', 'Event Planning', 'Leadership']
    },
    {
    ...
    },
    {
      ...
      current: false,
      description: [
        'Analyzed and processed large datasets using Python (Pandas, NumPy)',
        ...
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex gap-6">
              {['About', 'Experience', 'Projects', 'Publications', 'Certifications', 'Skills', 'Contact'].map((item) => (
                <button
                  key={item}
              ...
              {experiences.map((exp, index) => (
                <div key={index} className="relative pl-6 border-l-2 border-gray-300">
                  ...
                  <div className="flex items-center gap-3 text-sm text-gray-600 mt-1">
                    ...
                    {exp.current && <Badge className="bg-green-100 text-green-700 text-xs">Current</Badge>}
                  </div>
                ...
                <Card key={index} className="border-gray-200 hover:shadow-md transition-shadow">
                  <CardHeader>
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <Badge className="bg-gray-100 text-gray-700 text-xs">{project.period}</Badge>
                    </div>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex flex-wrap gap-2">
                      {project.technologies.map((tech) => (
                        <Badge key={tech} className="bg-blue-50 text-blue-700 text-xs">{tech}</Badge>
                      ))}
                    </div>
                    ...
                    <div key={category} className={activeSkillTab === category ? 'block' : 'hidden'}>
                      <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                        {skillCategories[category].skills.map((skill) => {
                          ...
                          return (
                            <div key={skill} className={`${skillData.bg} border border-gray-200 rounded-lg p-3 text-center hover:border-blue-300 transition-all`}>
                              <div className="text-2xl mb-1">{skillData.icon}</div>
                              <div className="text-gray-900 text-xs font-medium">{skill}</div>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <CardHeader>
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        ...
                        <p className="text-gray-600 mt-1">{edu.location} · {edu.period} · GPA: {edu.gpa}</p>
                      </div>
                      {edu.current && <Badge className="bg-green-100 text-green-700 text-xs">In Progress</Badge>}
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                      {edu.coursework.map((course) => (
                        <Badge key={course} className="bg-gray-100 text-gray-700 text-xs">{course}</Badge>
                      ))}
                    </div>
                  </CardContent>
                </Card>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <Card className="border-gray-200">
                <CardContent className="p-5">
                  ...
                  <p className="text-gray-700 text-sm">Houston, TX</p>
                </CardContent>
              </Card>
            </div>
          </section>
